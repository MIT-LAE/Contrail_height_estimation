...
class RegressionMLP(nn.Module):
    ...
    """ 
    def __init__(self, in_dim, out_dim, hidden_layer_sizes, activation="relu", dropout_p=None, batch_norm=False, bias=False):
        super().__init__()
        ...
        #self.layers.apply(torch.nn.init.)
        ...
        for layer in self.layers:
            ...
            if activation in ["relu", "leaky_relu"]:
                nn.init.kaiming_uniform_(layer.layers[0].weight, mode='fan_in', nonlinearity='relu')
            else:
                nn.init.xavier_normal_(layer.layers[0].weight)
            #nn.init.xavier_uniform_(layer.layers[0].weight.data, gain=gain)

            try:
                nn.init.zeros_(layer.layers[0].bias.data)
